...
public sealed class StorageCoreLibrary : StorageCorePlayableCollectionGroupBase, ICoreLibrary
{
    private readonly SemaphoreSlim _initSemaphore = new(1, 1);
    private FileMetadataManager? _fileMetadataManager;

    /// <summary>
    /// Initializes a new instance of the <see cref="StorageCoreLibrary"/> class.
    /// </summary>
    /// <param name="sourceCore">The core that created this instance.</param>
    public StorageCoreLibrary(ICore sourceCore)
        : base(sourceCore)
    {
        AttachEvents();
    }
    ...
    {
        base.TracksCountChanged += StorageCoreLibrary_TracksCountChanged;
        base.ArtistItemsCountChanged += StorageCoreLibrary_ArtistItemsCountChanged;
        base.AlbumItemsCountChanged += StorageCoreLibrary_AlbumItemsCountChanged;
        base.PlaylistItemsCountChanged += StorageCoreLibrary_PlaylistItemsCountChanged;
        base.ImagesCountChanged += StorageCoreLibrary_ImagesCountChanged;
    }
    ...
    private void AttachEvents(FileMetadataManager fileMetadataManager)
    {
    ...
    private void DetachEvents(FileMetadataManager fileMetadataManager)
    {
        fileMetadataManager.Tracks.MetadataAdded -= Tracks_MetadataAdded;
        fileMetadataManager.Albums.MetadataAdded -= Albums_MetadataAdded;
        fileMetadataManager.AlbumArtists.MetadataAdded -= Artists_MetadataAdded;
    ...
    private void StorageCoreLibrary_PlaylistItemsCountChanged(object sender, int e)
    {
        PlaylistItemsCountChanged?.Invoke(sender, e);
    }

    private void StorageCoreLibrary_AlbumItemsCountChanged(object sender, int e)
    {
        AlbumItemsCountChanged?.Invoke(sender, e);
    }

    private void StorageCoreLibrary_ArtistItemsCountChanged(object sender, int e)
    {
        ArtistItemsCountChanged?.Invoke(sender, e);
    }

    private void StorageCoreLibrary_ImagesCountChanged(object sender, int e)
    {
        ImagesCountChanged?.Invoke(this, e);
    }

    private void StorageCoreLibrary_TracksCountChanged(object sender, int e)
    {
    ...
    private void Playlists_MetadataAdded(object sender, IEnumerable<PlaylistMetadata> e)
    {
        ...
        foreach (var item in e)
        {
            ...
            addedItems.Add(new CollectionChangedItem<ICorePlaylistCollectionItem>(new StorageCorePlaylist(SourceCore, item), addedItems.Count));
        }
    ...
    private void Tracks_MetadataAdded(object sender, IEnumerable<TrackMetadata> e)
    {
        ...
        foreach (var item in e)
        {
            ...
            addedItems.Add(new CollectionChangedItem<ICoreTrack>(new StorageCoreTrack(SourceCore, item), addedItems.Count));
        }
    ...
    private void Artists_MetadataAdded(object sender, IEnumerable<ArtistMetadata> e)
    {
        ...
        foreach (var item in e)
        {
            ...
            addedItems.Add(new CollectionChangedItem<ICoreArtistCollectionItem>(new StorageCoreArtist(SourceCore, item), addedItems.Count));
        }
    ...
    private void Albums_MetadataAdded(object sender, IEnumerable<AlbumMetadata> e)
    {
        ...
        {
            Guard.IsNotNullOrWhiteSpace(item.Id, nameof(item.Id));
            addedItems.Add(new CollectionChangedItem<ICoreAlbumCollectionItem>(new StorageCoreAlbum(SourceCore, item), addedItems.Count));
        }
    ...
    private void Tracks_MetadataRemoved(object sender, IEnumerable<TrackMetadata> e)
    {
        ...
        foreach (var item in e)
        {
            ...
            removedItems.Add(new CollectionChangedItem<ICoreTrack>(new StorageCoreTrack(SourceCore, item), removedItems.Count));
            TracksChanged?.Invoke(this, addedItems, removedItems);
        }
    }
    ...
    private void Artists_MetadataRemoved(object sender, IEnumerable<ArtistMetadata> e)
    {
        ...
        foreach (var item in e)
        {
            ...
            removedItems.Add(new CollectionChangedItem<ICoreArtistCollectionItem>(new StorageCoreArtist(SourceCore, item), removedItems.Count));
            ArtistItemsChanged?.Invoke(this, addedItems, removedItems);
        }
    }
    ...
    private void Albums_MetadataRemoved(object sender, IEnumerable<AlbumMetadata> e)
    {
        ...
        foreach (var item in e)
        {
            ...
            removedItems.Add(new CollectionChangedItem<ICoreAlbumCollectionItem>(new StorageCoreAlbum(SourceCore, item), removedItems.Count));
    ...
    public override async IAsyncEnumerable<ICorePlaylistCollectionItem> GetPlaylistItemsAsync(int limit, int offset, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        foreach (var playList in playlistsMetadata)
        {
            ...
            yield return new StorageCorePlaylist(SourceCore, playList);
        }
    }
